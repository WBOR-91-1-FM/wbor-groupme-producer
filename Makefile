IMAGE_NAME = wbor-groupme-producer-image
CONTAINER_NAME = wbor-groupme-producer
NETWORK_NAME = wbor-network
HOST_DIR = "/var/wbor-groupme-producer"
...
run: stop
	docker run -d -p 49200:5000 --name $(CONTAINER_NAME) -v ${HOST_DIR}/logs:/app/logs $(IMAGE_NAME)
	# docker run -d --network $(NETWORK_NAME) -p 49200:5000 --name $(CONTAINER_NAME) -v ${HOST_DIR}/logs:/app/logs $(IMAGE_NAME)
